...
def make_dict(query_result, label):
    data = []
...
def get_most_recent_date():
    newest_date = session.query(Measurement).\
        order_by(Measurement.date.desc()).limit(1)

    for date in newest_date:
...
def return_precipitation():
    ...
    return jsonify(make_dict(recent_prcp_data, label='prcp'))
...
def return_tobs():
    ...
    return jsonify(make_dict(recent_tobs_data, label='tobs'))
